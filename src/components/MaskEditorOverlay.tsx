...
// 1. AFEGIT: Hem inclòs l'overlay (canvas visual) a l'historial
...
export function MaskEditorOverlay({
  ...
  const drawResultOnDisplay = useCallback((result: ImageData) => {
    ...
    // 1. Dibuixem el taulell d'escacs (El buit total)
    const TILE = 14;
    for (let y = 0; y < canvas.height; y += TILE) {
      for (let x = 0; x < canvas.width; x += TILE) {
        ctx.fillStyle = (x / TILE + y / TILE) % 2 === 0 ? '#1a2235' : '#111827';
        ctx.fillRect(x, y, TILE, TILE);
      }
    }

    // 2. Dibuixem la IMATGE FANTASMA (Original en blanc i negre i atenuada)
    if (originalCanvasRef.current) {
      ctx.save();
      ctx.globalAlpha = 0.25; // Ho fem gairebé transparent
      ctx.filter = 'grayscale(100%)'; // En blanc i negre per diferenciar-ho clarament
      ctx.drawImage(originalCanvasRef.current, 0, 0);
      ctx.restore();
    }

    // 3. Dibuixem el retall actual (A color i 100% opac) a sobre
    // Utilitzem un canvas temporal en lloc de putImageData perquè l'Alpha (transparència) funcioni bé
  ...
  // 2. AFEGIT: Capturem l'estat del canvas visual (overlay) juntament amb les màscares
  ...
  // 3. AFEGIT: L'estat inicial també ha de guardar la transparència del visual
  useEffect(() => {
    const { restoreCanvas, eraseCanvas } = getHiddenCanvases();
    const overlayCanvas = overlayCanvasRef.current;
  ...
  // 4. AFEGIT: En fer Ctrl+Z, apliquem també la imatge visual a l'overlayCanvas
  const applyHistoryStep = useCallback((targetIndex: number) => {
    ...
    overlayCanvas?.getContext('2d')?.putImageData(step.overlay, 0, 0); // Repintem el verd/vermell
...
const handleSmartDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const oppositeCanvas = brushMode === 'restore' ? eraseCanvas : restoreCanvas; // <-- AFEGIT
    ...
    // AFEGIT: Enforçar l'exclusivitat mútua (el que pintem aquí s'esborra de l'altre costat)
  ...
  const smartPointerMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    // AFEGIT: Enforçar l'exclusivitat mútua
    const oppCtx = oppositeCanvas.getContext('2d');
    if (oppCtx) {
      oppCtx.save();
      oppCtx.globalCompositeOperation = 'destination-out';
  ...
  const getCursor = (): string => {
    ...
    if (activeTool === 'brush') {
      ...
      const fop = brushMode === 'restore' ? '0.10' : '0.08';
  ...
  return (
    <div className="flex h-full flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-white/[0.07] bg-white/[0.03] px-3 py-2 backdrop-blur-sm">
        <div className="flex items-center gap-0.5 rounded-lg border border-white/[0.06] bg-black/20 p-0.5">
          <ToolBtn active={brushMode === 'restore'} activeClass="bg-emerald-500/20 text-emerald-300" onClick={() => setBrushMode('restore')}>
            <span className="text-[10px]">✦</span> Restore
          </ToolBtn>
          <ToolBtn active={brushMode === 'erase'} activeClass="bg-rose-500/20 text-rose-300" onClick={() => setBrushMode('erase')}>
        ...
        <div className="flex items-center gap-0.5 rounded-lg border border-white/[0.06] bg-black/20 p-0.5">
          <button type="button" onClick={undo} disabled={!historyState.canUndo} title="Desfer (Ctrl+Z)" className="flex h-6 w-6 items-center justify-center rounded-md text-slate-400 hover:bg-white/[0.08] hover:text-slate-100 disabled:opacity-25 disabled:hover:bg-transparent disabled:hover:text-slate-400 transition">
          ...
          <button type="button" onClick={redo} disabled={!historyState.canRedo} title="Refer (Ctrl+Shift+Z)" className="flex h-6 w-6 items-center justify-center rounded-md text-slate-400 hover:bg-white/[0.08] hover:text-slate-100 disabled:opacity-25 disabled:hover:bg-transparent disabled:hover:text-slate-400 transition">
        ...
        <div className="flex items-center gap-0.5 rounded-lg border border-white/[0.06] bg-black/20 p-0.5">
          <ToolBtn active={activeTool === 'brush'} activeClass="bg-cyan-500/20 text-cyan-300" onClick={() => setActiveTool('brush')} title="Freehand brush">
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg>
            Brush
          </ToolBtn>
          <ToolBtn active={activeTool === 'smart'} activeClass="bg-cyan-500/20 text-cyan-300" onClick={() => setActiveTool('smart')} title="Smart brush — selects by colour similarity">
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275Z"/></svg>
            Smart
          </ToolBtn>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-[9px] font-medium uppercase tracking-widest text-slate-600">Size</span>
          <div className="flex items-center gap-1">
            {BRUSH_SIZES.map((size) => (
              <button key={size} type="button" onClick={() => setBrushSize(size)} title={`${size}px`} className={`flex h-6 w-6 items-center justify-center rounded-full transition-all ${brushSize === size ? 'bg-cyan-400/15 ring-1 ring-cyan-400/50' : 'hover:bg-white/[0.05]'}`}>
                <span className={`rounded-full transition-colors ${brushSize === size ? 'bg-cyan-400' : 'bg-slate-600'}`} style={{ width: Math.max(3, Math.min(14, size / 5)), height: Math.max(3, Math.min(14, size / 5)) }} />
              </button>
            ))}
          </div>
        </div>
        ...
        {activeTool === 'smart' && (
          <>
            <div className="h-5 w-px bg-white/[0.08]" />
            <div className="flex items-center gap-1.5">
              <span className="text-[9px] font-medium uppercase tracking-widest text-slate-600">Tolerance</span>
              <div className="flex items-center gap-0.5">
                {TOLERANCE_PRESETS.map(({ label, value }) => (
                  <button key={value} type="button" onClick={() => setTolerance(value)} title={`RGB distance ≤ ${value}`} className={`rounded-md px-2 py-1 text-[10px] font-medium transition-all ${tolerance === value ? 'bg-cyan-400/15 text-cyan-300 ring-1 ring-cyan-400/40' : 'text-slate-500 hover:text-slate-300'}`}>
                    {label}
                  </button>
                ))}
              </div>
        ...
        {hasEdits && (
          <button type="button" onClick={handleClear} className="ml-auto rounded-lg px-2.5 py-1.5 text-[10px] font-medium text-slate-500 ring-1 ring-white/[0.06] transition hover:text-rose-400 hover:ring-rose-500/30">
            Clear
          </button>
        )}
      </div>

      <div className="relative min-h-0 flex-1 overflow-hidden rounded-xl border border-white/[0.06] flex items-center justify-center bg-black/20" style={{ touchAction: 'none' }}>
        <div className="relative" style={{ aspectRatio: `${aiResultImageData.width} / ${aiResultImageData.height}`, maxWidth: '100%', maxHeight: '100%' }}>
          <canvas ref={displayCanvasRef} className="block w-full h-full" style={{ imageRendering: 'pixelated' }} />
          <canvas ref={overlayCanvasRef} className="absolute inset-0 w-full h-full" style={{ cursor: getCursor(), touchAction: 'none' }} {...canvasProps} />
        </div>

        <div className="pointer-events-none absolute bottom-3 right-3">
          <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-medium backdrop-blur-md ${brushMode === 'restore' ? 'border-emerald-500/25 bg-emerald-500/10 text-emerald-300' : 'border-rose-500/25 bg-rose-500/10 text-rose-300'}`}>
...
function ToolBtn({ children, active, activeClass, onClick, title }: { children: React.ReactNode; active: boolean; activeClass: string; onClick: () => void; title?: string; }) {
  return (
    <button type="button" onClick={onClick} title={title} className={`flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-[11px] font-medium transition-all ${active ? activeClass : 'text-slate-500 hover:text-slate-300'}`}>
      {children}
    </button>
  );
}